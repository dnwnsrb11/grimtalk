  ...
  const applyMessageStyles = () => {
    ...
    entries.forEach((entry) => {
      ...
      if (nameElement) {
        ...
        // 방장 이름인 경우 크라운 아이콘 추가 (isCreator 조건 제거)
        if (userName === participantIdentity) {
          // 이미 크라운이 있는지 확인
          const existingCrown = entry.querySelector('.crown-icon');
          if (!existingCrown) {
            const crownContainer = document.createElement('span');
            crownContainer.className = 'crown-icon';
